...
public class DataverseLinkingServiceBean implements java.io.Serializable {
    ...
    public List<Dataverse> findLinkedDataverses(Long linkingDataverseId) {
        ...
        Query query = emBean.getMaster().createQuery("select object(o) from DataverseLinkingDataverse as o where o.linkingDataverse.id =:linkingDataverseId order by o.id");
    ...
    public List<Dataverse> findLinkingDataverses(Long dataverseId) {
        List<Dataverse> retList = new ArrayList<>();
        Query query = emBean.getMaster().createQuery("select object(o) from DataverseLinkingDataverse as o where o.dataverse.id =:dataverseId order by o.id");
    ...
    public void save(DataverseLinkingDataverse dataverseLinkingDataverse) {
        if (dataverseLinkingDataverse.getId() == null) {
            emBean.getMaster().persist(dataverseLinkingDataverse);
        } else {
            emBean.getMaster().merge(dataverseLinkingDataverse);
        }
    }
    ...
    public DataverseLinkingDataverse findDataverseLinkingDataverse(Long dataverseId, Long linkingDataverseId) {
        ...
        try {
            foundDataverseLinkingDataverse = emBean.getMaster().createQuery("SELECT OBJECT(o) FROM DataverseLinkingDataverse AS o WHERE o.linkingDataverse.id = :linkingDataverseId AND o.dataverse.id = :dataverseId", DataverseLinkingDataverse.class)
    ...
    public boolean alreadyLinked(Dataverse definitionPoint, Dataverse dataverseToLinkTo) {
        TypedQuery<DataverseLinkingDataverse> typedQuery = emBean.getMaster().createQuery("SELECT OBJECT(o) FROM DataverseLinkingDataverse AS o WHERE o.linkingDataverse.id = :dataverseId AND o.dataverse.id = :dataverseToLinkTo", DataverseLinkingDataverse.class);
