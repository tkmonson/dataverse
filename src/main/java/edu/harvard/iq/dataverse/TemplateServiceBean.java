...
public class TemplateServiceBean {
    ...
    public Template save(Template template) {
        /*
            ...
            emBean.getMaster().persist(template);
            return template;
        } else {
            return emBean.getMaster().merge(template);
        } */
        return emBean.getMaster().merge(template);
    }

    public Template findByDeafultTemplateOwnerId(Long ownerId) {
        TypedQuery<Template> query = emBean.getMaster().createQuery("select object(o.defaultTemplate) from Dataverse as o where o.owner.id =:ownerId order by o.name", Template.class);
    ...
    public List<Dataverse> findDataversesByDefaultTemplateId(Long defaultTemplateId) {
        TypedQuery<Dataverse> query = emBean.getMaster().createQuery("select object(o) from Dataverse as o where o.defaultTemplate.id =:defaultTemplateId order by o.name", Dataverse.class);
    ...
    public void incrementUsageCount(Long templateId) {
        ...
        emBean.getMaster().merge(toUpdate);
