...
public class SavedSearchServiceBean {
    ...
    public SavedSearch find(long id) {
        TypedQuery<SavedSearch> typedQuery = emBean.getMaster().createQuery("SELECT OBJECT(o) FROM SavedSearch AS o WHERE o.id = :id", SavedSearch.class);
    ...
    public List<SavedSearch> findAll() {
        TypedQuery<SavedSearch> typedQuery = emBean.getMaster().createQuery("SELECT OBJECT(o) FROM SavedSearch AS o ORDER BY o.id", SavedSearch.class);
    ...
    public SavedSearch add(SavedSearch toPersist) {
        ...
        try {
            persisted = emBean.getMaster().merge(toPersist);
        } catch (Exception ex) {
    ...
    public SavedSearch save(SavedSearch savedSearch) {
        if (savedSearch.getId() == null) {
            emBean.getMaster().persist(savedSearch);
            return savedSearch;
        } else {
            return emBean.getMaster().merge(savedSearch);
        }
    }
