...
public class VariableServiceBean {
    ...
    public DataVariable save(DataVariable variable) {
        DataVariable savedVariable = emBean.getMaster().merge(variable);
    ...
    public List<DataVariable> findByDataFileId(Long fileId) {
         TypedQuery<DataVariable> query = emBean.getMaster().createQuery("select object(o) from DataVariable as o where o.dataTable.dataFile.id =:fileId order by o.fileOrder", DataVariable.class);
         query.setParameter("fileId", fileId);
         return query.getResultList();
    }
    
    public List<DataVariable> findByDataTableId(Long dtId) {
         TypedQuery<DataVariable> query = emBean.getMaster().createQuery("select object(o) from DataVariable as o where o.dataTable.id =:dtId order by o.fileOrder", DataVariable.class);
    ...
    public VariableFormatType findVariableFormatTypeByName(String name) {
        Query query = emBean.getMaster().createQuery("SELECT t from VariableFormatType t where t.name = :name");
    ...
    public VariableIntervalType findVariableIntervalTypeByName(String name) {
        ...
        try {
            type=(VariableIntervalType)emBean.getMaster().createQuery(query).getSingleResult();
        } catch (javax.persistence.NoResultException e) {
            // DO nothing, just return null.
        }
        return type;
    ...
    public SummaryStatisticType findSummaryStatisticTypeByName(String name) {
        ...
        try {
            type = (SummaryStatisticType) emBean.getMaster().createQuery(query).getSingleResult();
        } catch (javax.persistence.NoResultException e) {
            // DO nothing, just return null.
        }
        return type;
    ...
    public List<SummaryStatisticType> findAllSummaryStatisticType() {
        ...
        return emBean.getMaster().createQuery(query).getResultList();
