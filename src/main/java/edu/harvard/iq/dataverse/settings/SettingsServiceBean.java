...
//        PVValidatorExpirationMaxLength,
    ...
    public void delete( String name ) {
        actionLogSvc.log( new ActionLogRecord(ActionLogRecord.ActionType.Setting, "delete")
                            .setInfo(name));
        emBean.getMaster().createNamedQuery("Setting.deleteByName")
                .setParameter("name", name)
                .executeUpdate();
    }
    
    public Set<Setting> listAll() {
        return new HashSet<>(emBean.getMaster().createNamedQuery("Setting.findAll", Setting.class).getResultList());
    }
