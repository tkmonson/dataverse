...
public class DatasetLinkingServiceBean implements java.io.Serializable {
    ...
    public List<Dataset> findLinkedDataverses(Long linkingDataverseId) {
        ...
        Query query = emBean.getMaster().createQuery("select object(o.dataverse.id) from DatasetLinkingDataverse as o where o.linkingDataverse.id =:linkingDataverseId order by o.id");
    ...
    public List<Dataset> findDatasetsThisDataverseIdHasLinkedTo(Long dataverseId) {
        ...
        TypedQuery<DatasetLinkingDataverse> typedQuery = emBean.getMaster().createQuery("SELECT OBJECT(o) FROM DatasetLinkingDataverse AS o WHERE o.linkingDataverse.id = :dataverseId", DatasetLinkingDataverse.class);
    ...
    public DatasetLinkingDataverse findDatasetLinkingDataverse(Long datasetId, Long linkingDataverseId) {
        ...
        try {
            foundDatasetLinkingDataverse = emBean.getMaster().createQuery("SELECT OBJECT(o) FROM DatasetLinkingDataverse AS o WHERE o.linkingDataverse.id = :dataverseId AND o.dataset.id = :datasetId", DatasetLinkingDataverse.class)
    ...
    public List<DatasetLinkingDataverse> findDatasetLinkingDataverses(Long datasetId) {
        return emBean.getMaster().createQuery("select object(o) from DatasetLinkingDataverse as o where o.dataset.id =:datasetId order by o.id", DatasetLinkingDataverse.class)
                .setParameter("datasetId", datasetId)
                .getResultList();
    }
    ...
    public void save(DatasetLinkingDataverse datasetLinkingDataverse) {
        if (datasetLinkingDataverse.getId() == null) {
            emBean.getMaster().persist(datasetLinkingDataverse);
        } else {
            emBean.getMaster().merge(datasetLinkingDataverse);
        }
    }
    
    public boolean alreadyLinked(Dataverse dataverse, Dataset dataset) {
        TypedQuery<DatasetLinkingDataverse> typedQuery = emBean.getMaster().createQuery("SELECT OBJECT(o) FROM DatasetLinkingDataverse AS o WHERE o.linkingDataverse.id = :dataverseId AND o.dataset.id = :datasetId", DatasetLinkingDataverse.class);
