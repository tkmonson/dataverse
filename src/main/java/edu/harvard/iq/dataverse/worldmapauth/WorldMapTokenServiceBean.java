...
public class WorldMapTokenServiceBean {
    ...
    public void expireToken(WorldMapToken wmToken){
        ...
        wmToken.expireToken();
        emBean.getMaster().merge(wmToken);     
    }
    ...
    public void deleteToken(WorldMapToken wmToken){
        ...
        emBean.getMaster().remove(emBean.getMaster().merge(wmToken));
    }
    ...
    */
    public void deleteExpiredTokens(){
        ...
        TypedQuery<WorldMapToken> query = emBean.getMaster().createQuery("select object(w) from WorldMapToken as w where w.hasExpired IS TRUE", WorldMapToken.class);// order by o.name");
        List<WorldMapToken> tokenList = query.getResultList();
        for (WorldMapToken wmToken : tokenList) {
           // emBean.getMaster().remove(token);
            emBean.getMaster().remove(emBean.getMaster().merge(wmToken));
	}
    }
    ...
    public WorldMapToken findByName(String token){
        ...
        try{
            return emBean.getMaster().createQuery("select m from WorldMapToken m WHERE m.token=:token", WorldMapToken.class)
    ...
    public WorldMapToken retrieveAndRefreshValidToken(String worldmapTokenParam){
        ...
        if (wmToken.hasTokenExpired()){
            emBean.getMaster().remove(emBean.getMaster().merge(wmToken));   // Delete expired token from the database.
