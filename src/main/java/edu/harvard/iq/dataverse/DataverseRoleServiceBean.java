...
public class DataverseRoleServiceBean implements java.io.Serializable {
    ...
    public DataverseRole save(DataverseRole aRole) {
        if (aRole.getId() == null) {
            emBean.getMaster().persist(aRole);
            /**
            ...
            DataverseRole merged = emBean.getMaster().merge(aRole);
            /**
    ...
    public RoleAssignment save(RoleAssignment assignment, boolean createIndex) {
        if (assignment.getId() == null) {
            emBean.getMaster().persist(assignment);
        } else {
            assignment = emBean.getMaster().merge(assignment);
        }
        /**
         * @todo update permissionModificationTime here.
         */
    ...
    public List<DataverseRole> findAll() {
        return emBean.getMaster().createNamedQuery("DataverseRole.listAll", DataverseRole.class).getResultList();
    }

    public void delete(Long id) {
        emBean.getMaster().createNamedQuery("DataverseRole.deleteById", DataverseRole.class)
            .setParameter("id", id)
            .executeUpdate();
    }

    public List<DataverseRole> findByOwnerId(Long ownerId) {
        return emBean.getMaster().createNamedQuery("DataverseRole.findByOwnerId", DataverseRole.class)
            .setParameter("ownerId", ownerId)
            .getResultList();
    }

    public List<DataverseRole> findBuiltinRoles() {
        return emBean.getMaster().createNamedQuery("DataverseRole.findBuiltinRoles", DataverseRole.class)
            .getResultList();
    }

    public DataverseRole findBuiltinRoleByAlias(String alias) {
        return emBean.getMaster().createNamedQuery("DataverseRole.findBuiltinRoleByAlias", DataverseRole.class)
    ...
    public void revoke(Set<DataverseRole> roles, RoleAssignee assignee, DvObject defPoint) {
        for (DataverseRole role : roles) {
            emBean.getMaster().createNamedQuery("RoleAssignment.deleteByAssigneeIdentifier_RoleIdDefinition_PointId")
                .setParameter("assigneeIdentifier", assignee.getIdentifier())
                .setParameter("roleId", role.getId())
                .setParameter("definitionPointId", defPoint.getId())
                .executeUpdate();
            emBean.getMaster().refresh(role);
        }
        emBean.getMaster().refresh(assignee);
    }

    public void revoke(RoleAssignment ra) {
        if (!emBean.getMaster().contains(ra)) {
            ra = emBean.getMaster().merge(ra);
        }
        emBean.getMaster().remove(ra);
        /**
         * @todo update permissionModificationTime here.
         */
    ...
    public List<RoleAssignment> roleAssignments(Long roleId) {
        return emBean.getMaster().createNamedQuery("RoleAssignment.listByRoleId", RoleAssignment.class)
    ...
    public Set<RoleAssignment> rolesAssignments(DvObject dv) {
        ...
        while (!dv.isEffectivelyPermissionRoot()) {
            ras.addAll(emBean.getMaster().createNamedQuery("RoleAssignment.listByDefinitionPointId", RoleAssignment.class)
    ...
    public List<RoleAssignment> directRoleAssignments(RoleAssignee roas, DvObject dvo) {
        ...
        TypedQuery<RoleAssignment> query = emBean.getMaster().createNamedQuery(
