...
public class Admin extends AbstractApiBean {
	...
	public Response listAuthProviders() {
		return ok(emBean.getMaster().createNamedQuery("AuthenticationProviderRow.findAll", AuthenticationProviderRow.class)
	...
	public Response addProvider(AuthenticationProviderRow row) {
		try {
			...
			if (managed != null) {
				managed = emBean.getMaster().merge(row);
			} else {
				emBean.getMaster().persist(row);
				managed = row;
			}
	...
	public Response checkAuthenticationProviderEnabled(@PathParam("id") String id) {
		List<AuthenticationProviderRow> prvs = emBean.getMaster()
	...
	public Response clearMetricsCache() {
		emBean.getMaster().createNativeQuery("DELETE FROM metric").executeUpdate();
	...
	public Response clearMetricsCacheByName(@PathParam("name") String name) {
		Query deleteQuery = emBean.getMaster().createNativeQuery("DELETE FROM metric where metricname = ?");
