...
public class DatasetVersionServiceBean implements java.io.Serializable {
    ...
    public DatasetVersion findByFriendlyVersionNumber(Long datasetId, String friendlyVersionNumber) {
        ...
        if (majorVersionNumber != null && minorVersionNumber == null) {
            ...
            try {
                TypedQuery<DatasetVersion> typedQuery = emBean.getMaster().createQuery("SELECT v from DatasetVersion v where v.dataset.id = :datasetId  and v.versionNumber= :majorVersionNumber", DatasetVersion.class);
    ...
    private DatasetVersion getDatasetVersionByQuery(String queryString){
        ...
        try{
            Query query = emBean.getMaster().createNativeQuery(queryString, DatasetVersion.class);           
    ...
    public Long getThumbnailByVersionId(Long versionId) {
        ...
        // generated:
        try {
            thumbnailFileId = (Long) emBean.getMaster().createNativeQuery("SELECT df.id "
    ...
    private void assignDatasetThumbnailByNativeQuery(Long versionId, Long dataFileId) {
        try {
            emBean.getMaster().createNativeQuery("UPDATE dataset SET thumbnailfile_id=" + dataFileId + " WHERE id in (SELECT dataset_id FROM datasetversion WHERE id=" + versionId + ")").executeUpdate();
        } catch (Exception ex) {
    ...
    public void populateDatasetSearchCard(SolrSearchResult solrSearchResult) {
        ...
        try {
            if (datasetId != null) {
                searchResult = (Object[]) emBean.getMaster().createNativeQuery("SELECT t0.VERSIONSTATE, t1.ALIAS, t2.THUMBNAILFILE_ID, t2.USEGENERICTHUMBNAIL, t3.STORAGEIDENTIFIER FROM DATASETVERSION t0, DATAVERSE t1, DATASET t2, DVOBJECT t3 WHERE t0.ID = " 
                        + datasetVersionId 
                        + " AND t1.ID = " 
                        + dataverseId
                        + " AND t2.ID = "
                ...
                searchResult = (Object[]) emBean.getMaster().createNativeQuery("SELECT t0.VERSIONSTATE, t1.ALIAS FROM DATASETVERSION t0, DATAVERSE t1 WHERE t0.ID = " + datasetVersionId + " AND t1.ID = " + dataverseId).getSingleResult();
            }
        } catch (Exception ex) {
            return;
        }
    ...
    public List<String> getChecksumListForDatasetVersion(DatasetVersion datasetVersion) {
        ...
        Query nativeQuery = emBean.getMaster().createNativeQuery(query);
        ...
        if (!forceRecalculate) {
            ...
            if (previousDatasetVersion != null) {
                ...
                if (isFileUnfsIdentical(fileUnfsInVersion, fileUnfsInPreviousVersion)) {
                    ...
                    DatasetVersion saved = emBean.getMaster().merge(datasetVersion);
