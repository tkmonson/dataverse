...
public class IpGroupsServiceBean {
    ...
    public IpGroup store( IpGroup grp ) {
        ...
        if ( grp.getId() == null ) {
            ...
            actionLogSvc.log( alr.setActionSubType("ipUpdate") );
            return emBean.getMaster().merge(grp);
        }
    }
    ...
    public IpGroup getByGroupName( String alias ) {
        try {
            return emBean.getMaster().createNamedQuery("IpGroup.findByPersistedGroupAlias", IpGroup.class)
    ...
    public List<IpGroup> findAll() {
        return emBean.getMaster().createNamedQuery("IpGroup.findAll", IpGroup.class).getResultList();
    }
    
    public Set<IpGroup> findAllIncludingIp( IpAddress ipa ) {
        if ( ipa instanceof IPv4Address ) {
            IPv4Address ip4 = (IPv4Address) ipa;
            List<IpGroup> groupList = emBean.getMaster().createNamedQuery("IPv4Range.findGroupsContainingAddressAsLong", IpGroup.class)
            ...
            List<IpGroup> groupList = emBean.getMaster().createNamedQuery("IPv6Range.findGroupsContainingABCD", IpGroup.class)
