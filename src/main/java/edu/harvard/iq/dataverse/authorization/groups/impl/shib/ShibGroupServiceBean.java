...
public class ShibGroupServiceBean {
    ...
    public ShibGroup findById(Long id) {
        TypedQuery<ShibGroup> typedQuery = emBean.getMaster().createQuery("SELECT OBJECT(o) FROM ShibGroup o WHERE o.id = :id", ShibGroup.class);
    ...
    public List<ShibGroup> findAll() {
        TypedQuery<ShibGroup> typedQuery = emBean.getMaster().createQuery("SELECT OBJECT(o) FROM ShibGroup as o", ShibGroup.class);
    ...
    public ShibGroup save(String name, String shibIdpAttribute, String shibIdp) {
        ...
        emBean.getMaster().persist(institutionalGroup);
        emBean.getMaster().flush();
        ShibGroup merged = emBean.getMaster().merge(institutionalGroup);
    ...
    public Set<ShibGroup> findFor(AuthenticatedUser authenticatedUser) {
        ...
        if (shibIdp != null) {
             ...
             */
            TypedQuery<ShibGroup> typedQuery = emBean.getMaster().createQuery("SELECT OBJECT(o) FROM ShibGroup as o WHERE o.pattern =:shibIdP", ShibGroup.class);
    ...
    public boolean delete(ShibGroup doomed) throws Exception {
        ...
        if (assignments.isEmpty()) {
            emBean.getMaster().remove(doomed);
            actionLogSvc.log( alr );
            return true;
        } else {
            /**
