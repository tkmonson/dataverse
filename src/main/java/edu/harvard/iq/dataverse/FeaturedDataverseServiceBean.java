...
public class FeaturedDataverseServiceBean {
    ...
    public List<DataverseFeaturedDataverse> findByDataverseId(Long dataverseId) {
        ...
        return emBean.getMaster().createQuery(qr, DataverseFeaturedDataverse.class).setParameter("dataverseId", dataverseId).getResultList();
    }
    ...
    public List<Dataverse> findByDataverseIdQuick(Long dataverseId) {
         ...
         try {
             //searchResults = emBean.getMaster().createNativeQuery("SELECT id, alias, name FROM dataverse WHERE id IN (select featureddataverse_id from DataverseFeaturedDataverse where dataverse_id = "+dataverseId+" order by displayOrder)").getResultList();
             searchResults = emBean.getMaster().createNativeQuery("SELECT d.id, d.alias, d.name FROM dataverse d, DataverseFeaturedDataverse f WHERE f.featureddataverse_id = d.id AND f.dataverse_id = "+dataverseId+" order by f.displayOrder").getResultList();
         } catch (Exception ex) {
             return null;
         }
    ...
    public List<DataverseFeaturedDataverse> findByRootDataverse() {
        return emBean.getMaster().createQuery("select object(o) from DataverseFeaturedDataverse as o where o.dataverse.id = 1 order by o.displayOrder", DataverseFeaturedDataverse.class).getResultList();
    }

    public void delete(DataverseFeaturedDataverse dataverseFeaturedDataverse) {
        emBean.getMaster().remove(emBean.getMaster().merge(dataverseFeaturedDataverse));
    }
    
	public void deleteFeaturedDataversesFor( Dataverse d ) {
		emBean.getMaster().createNamedQuery("DataverseFeaturedDataverse.removeByOwnerId")
    ...
    public void create(int diplayOrder, Long featuredDataverseId, Long dataverseId) {
        ...
        emBean.getMaster().persist(dataverseFeaturedDataverse);
    }	
