...
//    private static final Logger logger = Logger.getLogger(DataverseFieldTypeInputLevelServiceBean.class.getCanonicalName());
    ...
    public List<DataverseFieldTypeInputLevel> findByDataverseId(Long dataverseId) {
        ...
        if (res == null) {
            res = emBean.getMaster().createNamedQuery("DataverseFieldTypeInputLevel.findByDataverseId", DataverseFieldTypeInputLevel.class)
    ...
    public List<DataverseFieldTypeInputLevel> findByDataverseIdAndDatasetFieldTypeIdList( Long dataverseId, List<Long> datasetFieldIdList){
        ...
        try{
            return emBean.getMaster().createNamedQuery("DataverseFieldTypeInputLevel.findByDataverseIdAndDatasetFieldTypeIdList", DataverseFieldTypeInputLevel.class)
    ...
    //    Query query = emBean.getMaster().createQuery("select object(o) from MapLayerMetadata as o where o.dataset=:dataset");// order by o.name");
    //    query.setParameter("dataset", dataset);
    
    public DataverseFieldTypeInputLevel findByDataverseIdDatasetFieldTypeId(Long dataverseId, Long datasetFieldTypeId) {
        Query query = emBean.getMaster().createNamedQuery("DataverseFieldTypeInputLevel.findByDataverseIdDatasetFieldTypeId", DataverseFieldTypeInputLevel.class);
    ...
    public void delete(DataverseFieldTypeInputLevel dataverseFieldTypeInputLevel) {
        emBean.getMaster().remove(emBean.getMaster().merge(dataverseFieldTypeInputLevel));
        cache.invalidate();
    }

    public void deleteFacetsFor(Dataverse d) {
        emBean.getMaster().createNamedQuery("DataverseFieldTypeInputLevel.removeByOwnerId")
    ...
    public void create(DataverseFieldTypeInputLevel dataverseFieldTypeInputLevel) {
        ...
        emBean.getMaster().persist(dataverseFieldTypeInputLevel);
    }
