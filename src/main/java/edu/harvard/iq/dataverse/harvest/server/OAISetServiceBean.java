...
public class OAISetServiceBean implements java.io.Serializable {
    ...
    public OAISet findBySpec(String spec) {
        ...
        try {
            oaiSet = (OAISet) emBean.getMaster().createQuery(query).setParameter("specName", spec).getSingleResult();
        } catch (Exception e) {
    ...
    public List<OAISet> findAll() {
        try {
            ...
            List<OAISet> oaiSets = emBean.getMaster().createQuery("select object(o) from OAISet as o order by o.name", OAISet.class).getResultList();
    ...
    public void remove(Long setId) {
        ...
        emBean.getMaster().createQuery("delete from OAIRecord hs where hs.setName = '" + oaiSet.getSpec() + "'", OAISet.class).executeUpdate();
        //OAISet merged = emBean.getMaster().merge(oaiSet);
        emBean.getMaster().remove(oaiSet);
    }
...
//        solrQuery.addFilterQuery(SearchFields.PUBLICATION_STATUS + ":" + IndexServiceBean.PUBLISHED_STRING);
    ...
    public void setUpdateInProgress(Long setId) {
        ...
        emBean.getMaster().refresh(oaiSet);
